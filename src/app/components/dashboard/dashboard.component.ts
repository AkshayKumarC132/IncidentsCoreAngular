...
export class DashboardComponent {
  ...
  ngOnInit(): void {
    ...
    const pieChartCanvas = document.querySelector('canvas.pie-chart') as HTMLCanvasElement;
    const barChartCanvas = document.querySelector('canvas.bar-chart') as HTMLCanvasElement;
    ...
    Chart.register(ArcElement, Tooltip, Legend, CategoryScale, LinearScale, BarController, BarElement, PieController);
  ...
  populateSeverityChart(severitySummary: any[]) {
    this.severityChartLabels = severitySummary.map((item) => item.severity__level);
